...
class TestAPIMetrics:
    ...
    @pytest.mark.skip(reason="Endpoint /metrics not implemented yet")
...
class TestAPIBatchEndpoint:
    ...
    def test_batch_with_empty_list(self, client):
        """Test batch with empty CPF list."""
        # Using the correct endpoint /multiple_classify with correct payload structure
        response = client.post("/multiple_classify", json={"cpfs": []})
        # Should return 200 with empty results or 503 if model not loaded
        # or 422 if validation fails for empty list
