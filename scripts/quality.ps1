...
try {
    ...
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & $Python -m mypy src/domain src/data/synthetic src/models/pd src/models/sicr src/models/lgd src/models/ead src/models/forward_looking src/ecl/calculation src/ecl/discounting src/ecl/overlays src/ecl/stage3 src/application/services src/infrastructure/configuration src/regulatory/traceability
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
