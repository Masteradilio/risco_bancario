...
UTC_NOW = datetime(2026, 7, 1, 12, tzinfo=UTC)
...
def test_contract_uses_decimal_money_and_half_even_rounding() -> None:
    contract = Contract(
        ...
        date(2027, 1, 1),
        Decimal("1000.125"),
        effective_interest_rate="0.123456789",
    )
...
def test_guarantee_and_cash_flow_normalize_values() -> None:
    guarantee = Guarantee(
        "G-1", "CT-1", GuaranteeType.REAL_ESTATE, REFERENCE_DATE, "250000.999", "0.8"
    )
...
def test_snapshot_requires_aware_timestamp_and_coherent_pd() -> None:
    snapshot = RiskSnapshot(
        "SN-1", "CT-1", REFERENCE_DATE, UTC_NOW, Stage.STAGE_2, 31, "0.10", "0.30", "staging-v1"
    )
    ...
    with pytest.raises(DomainValidationError, match="pd_lifetime"):
        RiskSnapshot(
            "SN-3", "CT-1", REFERENCE_DATE, UTC_NOW, Stage.STAGE_1, 0, "0.20", "0.10", "v1"
        )
...
def test_scenario_is_versioned_and_variables_are_unique() -> None:
    ...
    with pytest.raises(DomainValidationError, match="unique"):
        Scenario(
            ...
            "1",
            "v1",
            (MacroVariable("gdp", "1"), MacroVariable("gdp", "2")),
        )
