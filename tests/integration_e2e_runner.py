...
class IntegrationTestResults:
    ...
    def add_result(self, test_name: str, passed: bool, message: str = "", details: dict = None):
        ...
        self.results.append(
            {"test": test_name, "status": status, "message": message, "details": details or {}}
        )
...
def test_prinad_classifier():
    ...
    try:
        from prinad.src.classifier import ClassificationResult, PRINADClassifier
        ...
        results.add_result(
            ...
            {
                "prinad": result_low.prinad,
                "rating": result_low.rating,
                "stage": result_low.estagio_pe,
            },
        )
        ...
        results.add_result(
            ...
            isinstance(result_high, ClassificationResult)
            and result_high.prinad > result_low.prinad,
            f"PRINAD={result_high.prinad:.2f}%, Rating={result_high.rating}",
            {
                "prinad": result_high.prinad,
                "rating": result_high.rating,
                "stage": result_high.estagio_pe,
            },
        )
...
def test_perda_esperada_pipeline(prinad_result):
    ...
    try:
        from perda_esperada.src.pipeline_ecl import ECLCompleteResult, ECLPipeline

        pipeline = ECLPipeline()
        results.add_result(
            "ECL Pipeline Init", pipeline is not None, "Pipeline ECL inicializado com sucesso"
        )
...
def test_propensao_pipeline():
    ...
    try:
        ...
        results.add_result(
            "Propensao ECLCalculator Init", ecl_calc is not None, "ECLCalculator inicializado"
        )
        ...
        results.add_result(
            "Propensao Model Init", prop_model is not None, "PropensityModel inicializado"
        )
    ...
    try:
        ...
        for cenario in cenarios:
            ...
            try:
                ...
                prinad_ok = (
                    cenario["prinad_esperado"][0]
                    <= prinad_result.prinad
                    <= cenario["prinad_esperado"][1]
                )
                ...
                ecl_result = ecl_pipeline.calcular_ecl_de_prinad_result(
                    ...
                    limite_total=50000,
                    dias_atraso=int(
                        cenario["client_data"]["dados_comportamentais"].get("scr_dias_atraso", 0)
                    ),
                )
